...
local _Cursor = CreateFrame("Frame", "_Cursor")
...
-- Constants
-------------------------------------------------------------------------------
_Cursor.DefaultModelSet = L.SETS["Energy beam"]
_Cursor.DefaultType = ""
_Cursor.DefaultValue = "spells\\errorcube" -- Used when cursor preset not found
_Cursor.ScaleDefault = 0.01 -- Baseline scaling factor applied before presets and custom scales
...
local function EnableModel(model, cursorData)
	...
	local scale = (cursor.Scale or 1.0) * _Cursor.ScaleDefault
	...
	if #cursor.Type == 0 then
		...
		local presetType = _Cursor.Presets[cursor.Type]

		if not presetType or not presetType[cursor.Value] then
			cursor.Type = _Cursor.DefaultType
			cursor.Value = _Cursor.DefaultValue
			presetType = _Cursor.Presets[cursor.Type]
		end
...
-- Events
-------------------------------------------------------------------------------
_Cursor:RegisterEvent("ADDON_LOADED")
_Cursor:RegisterEvent("CINEMATIC_START")
_Cursor:RegisterEvent("CINEMATIC_STOP")
_Cursor:RegisterEvent("SCREENSHOT_FAILED")
_Cursor:RegisterEvent("SCREENSHOT_SUCCEEDED")

function _Cursor:SCREENSHOT_FAILED()
	RemoveHideCondition("Screenshot")
end

_Cursor.SCREENSHOT_SUCCEEDED = _Cursor.SCREENSHOT_FAILED
...
do
	-- Compares delimited version strings
	...
	function _Cursor:ADDON_LOADED(_, addonName)
		if addonName:lower() ~= FOLDER_NAME:lower() then
			return
		end

		_Cursor:UnregisterEvent("ADDON_LOADED")
		_Cursor.ADDON_LOADED = nil

		if not _CursorOptions then
			_CursorOptions = {
				Sets = CopyTable(_Cursor.DefaultSets)
			}
		end

		if _CursorOptions.Version ~= Version then
			if VersionCompare(_CursorOptions.Version, "3.1.0.2") < 0 then
				local Name = L.SETS["Face Melter (Warning, bright!)"]

				if _CursorOptions.Sets[Name] then
					_CursorOptions.Sets[Name] = CopyTable(_Cursor.DefaultSets[Name])
				end
			end
		...
		if not _CursorOptionsCharacter.Cursors[1] then
			...
			if _Cursor.Options then
				_Cursor.Options.Update()
			end
		end
	end
end
...
_Cursor:SetScript("OnEvent", function(self, eventName, ...)
	return self[eventName](self, eventName, ...)
end)

_Cursor:SetScript("OnShow", function()
	for model in pairs(ActiveModels) do
		if model.GetModel then
			-- Returns model table if unset
...
do
	...
	--This is the maximum position that the cursor can have in the vertical dimension.
	local cursorYRange = 768

	--This is the maximum position that the effect can have in the vertical dimension.
	local effectYRange = 60

	--"scale" allows us to convert between the cursor space and the effect space.
	local scale = cursorYRange / effectYRange
	local previousX, previousY

	_Cursor:SetScript("OnUpdate", function(self, elapsed)
		...
		for model in pairs(ActiveModels) do
			--  ...Even the offsets are offset?
			local offsetX = model.X - 6
			local offsetY = model.Y + 6

			-- Getting the effect size and removing the default that was applied earlier. This makes it so that changing the size of the effect doesn't shift it.
			local cursorScale = model:GetModelScale() / _Cursor.ScaleDefault

			--Converting the cursor position into effect space.
			local posX = (cursorX + offsetX) / scale / cursorScale
			local posY = (cursorY + offsetY) / scale / cursorScale
