...
function LeaderToolbar.CreateScreenPanel()
	...
	for i = 1, 8 do
		...
		local icon = LeaderToolbar:CreateImage (raid_marker_button, [[Interface\AddOns\IskarAssist\media\world_markers_icons]] , 18, 18, "overlay")
...
--	readycheck_timer = 35,	
	...
	local do_pull = function()
		if (_G.DBM and SlashCmdList ["DEADLYBOSSMODPULL"]) then
			SlashCmdList ["DEADLYBOSSMODPULL"] (tostring (LeaderToolbar.db.pull_timer))
	...
	local pull_button = LeaderToolbar:CreateButton (ScreenPanel, do_pull, 50, 20, "Pull", _, _, _, "pull_button", _, "none", button_template)
