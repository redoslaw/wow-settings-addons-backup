...
function QDKP2_CLI_ProcessCommand(text)
  ...
  --  Kluge command line for testing
  elseif (W1=="boss") then
	if (W2 == nil ) then
	  DEFAULT_CHAT_FRAME:AddMessage("Boss test with no boss")
	  return
	else
	  DEFAULT_CHAT_FRAME:AddMessage("Boss test with "..W2)
	  local awardtest = QDKP2_GetBossAward(W2) or "nil"
	  local instDiff, DKPType = QDKP2_InstanceDiff()
