...
local QDKP2export_GenericExporter={}
QDKP2export_GenericExporter.Formats={"Text","HTML","XML"}
function QDKP2export_GenericExporter:GetData(EntryList,Format)
	local rowList={}
...
--Export
function QDKP2export_GenericExporter:MakeTable(Format,rowList)
	local width={}
  ...
  elseif Format=='html' then
		...
		error "Invalid table format. Valid formats are text or html."
  end
end

function QDKP2export_GenericExporter:MakeXML(rowList)
	local out=''
...
--Column
function QDKP2export_GenericExporter:MakeHeader(Format,Header)
	local out=Header or ''
	if Format=='html' then
		out=out:gsub("<","&lt;")
		out=out:gsub(">","&gt;")
...
function QDKP2export_GenericExporter:MakeTail(Format,Tail)
	out=Tail or ''
  if Format=='text' then
    out=out..'\n[/CODE]'
  end
...
--DKP exporter for a generic list of players
QDKP2export_DKPExporter=QDKP2_CopyTable(QDKP2export_GenericExporter)
function QDKP2export_DKPExporter:Export(Format,optList)
  local nameList = self:GetMembers(optList)
  local out
...
function QDKP2export_DKPExporter:GetMembers(optList) return optList; end
QDKP2export_DKPExporter.XMLRowVoiceName="PLAYER"
QDKP2export_DKPExporter.Columns={
{
header = "Name",
xmlHeader="playername",
width = 28,
...
isDisabled = function(name,Format) if Format=="xml" then return false; else return true; end; end,
},
{
header = "Class",
xmlHeader="class",
...
--Guild roster DKP exporter
QDKP2export_GuildExporter=QDKP2_CopyTable(QDKP2export_DKPExporter)
function QDKP2export_GuildExporter:GetMembers() return QDKP2name; end

--Raid roster DKP exporter
 QDKP2export_RaidExporter=QDKP2_CopyTable(QDKP2export_DKPExporter)
function QDKP2export_RaidExporter:GetMembers() return QDKP2raid; end


--LogList exporter
QDKP2export_LogListExporter=QDKP2_CopyTable(QDKP2export_GenericExporter)
QDKP2export_LogListExporter.Formats={"Text","HTML"}
function QDKP2export_LogListExporter:Export(Format,LogList)
...
QDKP2export_LogListExporter.Columns={
{header="Time",
xmlHeader="timestamp",
width=12,
...
QDKP2export_LogSessionExporter=QDKP2_CopyTable(QDKP2export_LogListExporter)
function QDKP2export_LogSessionExporter:Export(Format,SID)
	local LogList=QDKP2log_GetPlayer(SID,"RAID")
	if not LogList then return ''; end
	return QDKP2export_LogListExporter:Export(Format,LogList)
end


--Extended Log Session exporter
QDKP2export_ExtLogSessionExporter=QDKP2_CopyTable(QDKP2export_LogListExporter)
function QDKP2export_ExtLogSessionExporter:Export(Format,SID)
	...
	--LogList=turn
	return QDKP2export_LogListExporter.Export(self,Format,LogList)
end

QDKP2export_ExtLogSessionExporter.Columns=QDKP2_CopyTable(QDKP2export_LogListExporter.Columns)
table.insert(QDKP2export_ExtLogSessionExporter.Columns,2,{
header="Player",
xmlHeader="player",
width=24,
query=function(log,Format)
...
local ExporterList={
list=QDKP2export_DKPExporter,
guild=QDKP2export_GuildExporter,
raid=QDKP2export_RaidExporter,
loglist=QDKP2export_LogListExporter,
logsession=QDKP2export_LogSessionExporter,
extlogsession=QDKP2export_ExtLogSessionExporter,
}
