...
function QDKP2_OnEvent(self, event, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9)
--Event manager
-- RECEIVER FOR THE MOBS DEATH EVENT  --Modified for 6.0 to use encounter end
--[[ Old version Note: re enable CLEU event in initialization if re-added
...
--]]  --End of old version
  ...
  if event == "ENCOUNTER_END" and QDKP2_ManagementMode() then
      local boss=arg2 or "NoBoss"
	  local instDiff = arg3 or "0"
	  local size = arg4 or "odd"
	  local success = arg5 or "NoSuccess"
	  --DEFAULT_CHAT_FRAME:AddMessage("Boss "..boss.." diff "..instDiff.." size"..size.." success "..success) 
      if boss and (success==1) then --removed and QDKP2_IsInBossTable(boss,instDiff) as unnecessary check 6.0
        QDKP2_Debug(2,"Core","SlainDetector triggers a boss kill:"..boss)
		--DEFAULT_CHAT_FRAME:AddMessage("Boss two "..boss.." diff "..instDiff.." success "..success) 
        QDKP2_BossKilled(boss, instDiff)
      end
    --end
    return
  end
...
-- RAID/PARTY ROSTER UPDATED
  elseif(event == "GROUP_ROSTER_UPDATE" ) then  --fired on raid members add/leave, maybe even on loot change
...
-- GUILD ROSTER UPDATED
  elseif(event ==  "GUILD_ROSTER_UPDATE") then  --fired when a new fresh guild cache is downloaded.
    if GetNumGuildMembers(true) > 0 and not InCombatLockdown() then
...
function QDKP2_OnGroupChat(arg1,arg2,arg3)
  ...
  if itemStr then
    ...
    if itemName then
      ...
      if QDKP2frame3_reasonBox and itemRarity >= MIN_LISTABLE_QUALITY then
        QDKP2frame3_reasonBox:AddHistoryLine(itemLink)
      end
    end
  end
  if QDKP2_GuildData.DetectBids then SearchForWinner(arg1); end
end
