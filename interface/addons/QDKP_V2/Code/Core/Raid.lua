...
function QDKP2_IsRaidPresent()
  if IsInRaid()  then return true  -- New with 5.0
  end
end
...
function QDKP2_GetRaidRosterInfo(i)
  ...
  local PartySize=GetNumSubgroupMembers()
  if RaidSize>0 then
    GroupSize=RaidSize
  elseif PartySize>0 then  -- should never be called in 5.0, leaving in
  ...
  if not (i > GroupSize) then
    if IsInRaid() then  -- New with 5.0
	  ...
	  --if (name2 ~= name) then 
	    DEFAULT_CHAT_FRAME:AddMessage("RosterName "..name.." UnitName "..name2.." realm2 "..realm2) 
	  --end;
	  --]]
    else
      local unit
