...
-- Starts a new bid. item is the string of the item to bid (or a generic reason)
  ...
  QDKP2_Events:Fire("BID_START")
  QDKP2_BidM_BidButtonsUpdate()
end

function QDKP2_BidM_CloseBidNow()
--stops the bidding. Does not clear the data.
  QDKP2_BidM.BIDDING = nil
  QDKP2_BidM_BidButtonsUpdate()
  QDKP2_Events:Fire("BID_CLOSE")
end

function QDKP2_BidM_CloseBid()
  if QDKP2_BidM_CountStop and QDKP2_BidM_isBidding() then QDKP2_BidM_Countdown(QDKP2_BidM_CloseBidNow)
  else QDKP2_BidM_CloseBidNow()
	end
end
...
-- Clears the data
  ...
  QDKP2_BidM_CloseBidNow()
  QDKP2_BidM.ITEM=nil
  QDKP2_Events:Fire("BID_CANCEL")
  QDKP2_BidM_Reset()
end
...
local function notify_winner(winner,sure)
  if not QDKP2_BidM.LIST[winner] then
    QDKP2_Debug(1,"BidManager","Called QDKP2_BidM_Winner with a winner that is not in the bid list")
    return
  end
  local bid=QDKP2_BidM.LIST[winner]
  if bid.value then
    if not QDKP2_UpdateBid(winner,bid,true) then
			QDKP2_Debug(1,"BidManager","Last bid update failed and returned false!! this can mean the DKP amount can be wrong.")
		end
  end
  local dkp = bid.dkp or bid.value

  local GiveLoot, err1
  if QDKP2_BidM.ITEM then GiveLoot, err1=QDKP2_GiveLootToPlayer(QDKP2_BidM.ITEM, winner, true); end
  if (not GiveLoot) then 
	QDKP2_Msg(err1,'ERROR')
	return false; 
  end
  if GiveLoot and not sure and QDKP2_BidM_ConfirmWin then
    local msg=QDKP2_LOC_BidGiveLoot
    msg=string.gsub(msg,"$ITEM",QDKP2_BidM.ITEM)
    msg=string.gsub(msg,"$NAME",winner)
    msg=string.gsub(msg,"$AMOUNT",tostring(math.floor(dkp or 0)))
    QDKP2_AskUser(msg,notify_winner,winner,true)
    return
  end

  if QDKP2_IsInGuild(winner) and dkp and dkp ~= 0 then
    dkp=math.floor(dkp)
    if GiveLoot or not QDKP2_BidM_ConfirmWin then
      QDKP2_PlayerSpends(winner, dkp, QDKP2_BidM.ITEM)
    else
  ...
  if QDKP2_BidM_AnnounceWinner and QDKP2_BidM.ITEM and #QDKP2_BidM.ITEM>0 then
    if dkp and dkp ~= 0 then
	  local karmatag = 0
	  if (bid_voice.dkp == nil) then 
	    karmatag = 0
	  elseif (bid_voice.dkp == "$net/2") then
	    karmatag = 1
	  end
	  ...
	  if (tempvalue > dkp and karmatag==1) then --Karma system report
	    msg=QDKP2_BidM_WinnerString
		msg=msg:gsub("$AMOUNT","karma "..tostring(bid.value))
	  else              -- Old lines
        msg=QDKP2_BidM_WinnerString
	...
	msg=msg:gsub("$NET",QDKP2_GetNet(winner))
  ...
  if QDKP2_IsInGuild(winner) and QDKP2_BidM_LogBids and QDKP2_BidM.ITEM and #QDKP2_BidM.ITEM>0  then
    ...
    if QDKP2_IsManagingSession() then QDKP2log_Link("RAID",winner,timestamp,QDKP2_OngoingSession()); end
		if QDKP2_BidM_GiveItemToWinner and QDKP2_IsMasterLooter() and GetNumLootItems() and GetItemInfo(QDKP2_BidM.ITEM or '') then
			local status,err = QDKP2_GiveLootToPlayer(QDKP2_BidM.ITEM, winner)
			if not status then QDKP2_Msg(err,'ERROR'); end
		end
    QDKP2_Events:Fire("DATA_UPDATED","log")
  end
  QDKP2_BidM_CloseBidNow()
...
function QDKP2_BidM_Winner(winner)
  if QDKP2_BidM_CountStop and QDKP2_BidM_isBidding() then QDKP2_BidM_Countdown(winner)
...
local function countdown_trigger()
	if QDKP2_BidM_CountdownWinner then
		if type(QDKP2_BidM_CountdownWinner)=="function" then QDKP2_BidM_CountdownWinner()
		else notify_winner(QDKP2_BidM_CountdownWinner)
		end
	end
end
...
-- Initializes a countdown, then annouce the winner (if given).
...
-- Watch for bids from raid. If detected, import them and do all the things as set in the configuration.
  ...
  --[[Debugging block for cross-realm
	...
	DEFAULT_CHAT_FRAME:AddMessage("BidWatcherName "..tostring(player)) 
...
function QDKP2_BidM_SendMessage(player, t,  channel, txt)
  ...
  if channel == "GROUP" then
    ...
    elseif GetNumSubgroupMembers()>0 then channel="PARTY"
...
function QDKP2_BidM_BidButtonsHandler(i)
  local lootB=getglobal("LootButton"..tostring(i))
  local item=GetLootSlotLink(lootB.slot)
  if item then
...
function QDKP2_BidM_BidButtonsUpdate()
  if QDKP2_IsManagingSession() then
	local imax = GetNumLootItems()
	if imax > 4 then imax = 3 end;
    for i=1, imax do
      local lootB=_G["LootButton"..tostring(i)]
      local bidB=_G["QDKP2_BidButtons_Slot"..tostring(i)]
      if (GetLootSlotType(i) == LOOT_SLOT_ITEM) and lootB.quality >= MIN_LOGGED_LOOT_QUALITY  then --error here replace GetLootThreshold()
        bidB:Show()
        if QDKP2_BidM_isBidding() then bidB:Disable()
        else bidB:Enable()
        end
      else
        bidB:Hide()
    ...
    for i=1,4 do _G["QDKP2_BidButtons_Slot"..tostring(i)]:Hide(); end
  end
end
...
--  else
--    for i=1,4 do getglobal("LootButton"..tostring(i)):Hide(); end
--    return
--  end
--  for i=1,4 do
--    local lootB=getglobal("LootButton"..tostring(i))
--    local bidB=getglobal("QDKP2_BidButtons_Slot"..tostring(i))
--    if LootSlotIsItem(i) and lootB.quality >= GetLootThreshold() then
--      bidB:Show()
--     if QDKP2_BidM_isBidding() then bidB:Disable()
--      else bidB:Enable()
--      end
...
function QDKP2_BidM_LootFrameHook(...)
  QDKP2_BidM_OrigLootFrameHook(...)
  QDKP2_BidM_BidButtonsUpdate()
end
