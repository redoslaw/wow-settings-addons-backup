...
--]]
  ...
  local Diff
  Diff = "Unk"
  _, Diff=QDKP2_InstanceDiff()  --Consolidate in boss.lua as of 6.0
  --[[
  Diff = "Unk"
  if     instDiff==3 then Diff="10"
  elseif instDiff==4 then Diff="25"
  elseif instDiff==5 then Diff="10H"
  elseif instDiff==6 then Diff="25H"
  elseif instDiff==7 then Diff="LFR"
...
function QDKP2_OfficerMode()
  ...
  --Modified for 5.0 to not require EditGuildInfo but only note editing privs
  --changed from negative to postitive logic
  if
  --CanEditGuildInfo() or
  (QDKP2_OfficerOrPublic==2 and  CanEditPublicNote()) or
  (QDKP2_OfficerOrPublic~=2 and  CanEditOfficerNote()) then
    return true
  end
  return false
end
