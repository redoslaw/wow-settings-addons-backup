...
function QDKP2_DownloadGuild(Revert)
  ...
  for i=1, QDKP2_GetNumGuildMembers(true) do
	...
	--[[Debugging block for cross-realm
	 if (name == "Marygwen-Hyjal") then
		DEFAULT_CHAT_FRAME:AddMessage("Guild Datafield "..datafield.." firstword "..Main) 
	 end
		--]]
    ...
    elseif (QDKP2_IsInGuild(Main) or QDKP2altsRestore[name]) and level>=QDKP2_MINIMUM_LEVEL and not Hide_Rank then
      if QDKP2altsRestore[name] then Main=QDKP2altsRestore[name]; end
...
function QDKP2_UploadAll()
  QDKP2_Debug(3, "Guild", "UploadAll called")
  ...
  for i=1, table.getn(QDKP2name) do
    ...
    if (QDKP2_IsModified(name) and not QDKP2_IsAlt(name)) or (QDKP2altsRestore[name] and QDKP2altsRestore[name]=="") then
      QDKP2_Debug(3,"Guild","Processing "..name.." because he has modificatione")
    ...
    elseif QDKP2altsRestore[name] and QDKP2altsRestore[name]~="" then
      QDKP2_Debug(3,"Guild","Processing "..name.." because is in the AltsRestore list")
...
function QDKP2_GetGuildRosterInfo(i)
  ...
  if not IsInGuild() then GuildSize=0; end                      --see QDKP2_GetNumGuildMembers()
  ...
  if i<=GuildSize then
	...
	--[[Debugging block for cross-realm
	    ...
	    DEFAULT_CHAT_FRAME:AddMessage("GuildRosterName "..name) 
