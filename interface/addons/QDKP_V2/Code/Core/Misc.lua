...
function QDKP2_MakeNotifyMsg(name,u3p)
  ...
  msg=string.gsub(msg,"$RATIO",tostring(QDKP2_GetTotSpentRatio(name))) -- Credit to Bryce
...
QDKP2_Debug_Subsystems={"General","Core","Logging","Guild","Raid","Session","Init","CLI","Log-DB","Update","Raidaward"}
...
-- true will show that subsystem, false will hide.
  System=string.lower(System)
	QDKP2_Debug_Filter=QDKP2_Debug_Filter or {}
...
--filter all
		QDKP2_Debug_Filter=QDKP2_Debug_Filter or {}
		for i,subs in pairs(QDKP2_Debug_Subsystems) do QDKP2_Debug_Filter[string.lower(subs)]=true
		end
  end
end
...
--If it finds a -, it assumes what follows is a realm name
  ...
  --assert(type(name)=='string')  --This line fails when swapping raid leader characters
  if type(name)~='string' then return ""; end  --Eliminate assertion error per Dracula
  if #name<1 then return ""; end
  if #name==1 then return string.upper(name).."-"..QDKP2_playerRealm; end
  --Use utf-8 code
  local first = string.utf8sub(name, 1, 1)
  local remainder = string.utf8sub(name, 2)
  ...
  --]]
  local output = string.utf8upper(first)  --Need to not lower case the realm name, done below in loop
  --local output = string.upper(first) --Leave utf8 alone, make sure other chars are uppercase
  ...
  local foundServer = 0
  for i=2,string.utf8len(name) do
    --local char=strbyte(name,i+1)
	local char = string.utf8sub(name, i, i)
	if not char then break; end
	if ( char == "-" ) then --was 45
	  foundServer = 1
	  --remainder = string.sub(name, i+1)  --Original code
	  remainder = string.utf8sub(name, i)
	  
	  output = output..remainder
	  break
	else -- for other characters in the name not in the realm name
	  output = output..string.utf8lower(char)
	  --[[
	  if (char < 128) then
...
function QDKP2_UTF8len(str)
  ...
  --[[ Old version replaced by UTF8 library
  l=0
  for i=1,#str do
    local char=strbyte(str,i)
    if char<128 or (char>193 and char<245) then l=l+1; end  --should probably have been 191
  end
  return l
  --]]
end
