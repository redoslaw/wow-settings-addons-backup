...
--Modified to use utf-8 encoding
   ...
   local first = string.utf8sub(str, 1, 1) --was string.sub(str, 1,1)
   local fin = string.find(str, " ") or (string.utf8len(str) + 1)  --since space is an ascii this should be fine, change length
   local remainder = string.utf8sub(str, 2, fin-1) --was string.sub(str, 2, fin - 1)
   --local output = string.upper(first)..string.lower(remainder)  --original line
   output = string.utf8upper(first)..(remainder)  --in order to keep server in correct capitalization
