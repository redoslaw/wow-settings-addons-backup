...
--returns a float. If Spent is 0 returns 9999
  ...
  local Spent=QDKP2_GetSpent(name,doNotReset)
  if Spent==0 then return 9999; end
  return (Total / Spent)
end
...
-- msgIt will display a text of the modification to the given channel.
  ...
  if type(award)=="string" then award=QDKP2_GetAmountFromText(award,name); end
  if type(spent)=="string" then spent=QDKP2_GetAmountFromText(spent,name); end
  ...
  if award then
    if award ~= 0 then
      ...
      if newTotal>limitMax then newTotal=limitMax
      elseif newTotal<limitMin then newTotal=limitMin
      end
  ...
  if spent then
    if spent ~= 0  then
      ...
      if newSpent>limitMax then newSpent=limitMax
      elseif newSpent<limitMin then newSpent=limitMin
      end
  ...
  if hours then
    if hours ~= 0 then
      ...
      elseif newHours > 9999.9 then newHours = 9999.9
      end
  ...
  if not HaveModify and not reason then return; end
...
function QDKP2_PlayerGains(name,amount,reason,NoLog,NoMsg)
  ...
  if QDKP2_SENDTRIG_MODIFY then QDKP2_UploadAll(); end
end
...
function QDKP2_GetAmountFromText(text,name)
  ...
  if string.sub(text, -1)=="%" then
    ...
    if perc then text=QDKP2_GetNet(name)*perc/100; end
