...
-- This function should be error-proof in most cases, since is called almost directly by both the GUI and the command line interface.
  ...
  if giverName and not QDKP2_IsChargeable(giverName, amount) then
    ...
    return
  elseif giverName and not QDKP2log[SID][QDKP2_GetMain(giverName)] then
  ...
  for i=1, QDKP2_GetNumRaidMembers() do
    ...
    if inguild and not removed and not QDKP2_IsMainAlreadyProcessed(name) then
      ...
      -- Checks if <name> is able to get the award.
			local eligible,percentage,noreason=QDKP2_GetEligibility(name,AwardType,amount,online,inzone) --checks if player is eligible.
				if eligible then
      ...
      elseif not QDKP2_AltsStillToCome(name, nameBase, i) and noreason then
      ...
      if (online or QDKP2_AWARD_OFFLINE_TIMER) and (inzone or QDKP2_AWARD_ZONE_TIMER) then
        ...
        if NowHours ~= OrigHours then  --use this to detect if i've hit an integer (eg 2.9 + 0.2 = 3.1)
          QDKP2_Debug(3,"Timer","It's the hour for "..tostring(name))
					local eligible,percentage,noreason=QDKP2_GetEligibility(name,'timer',QDKP2timerBase.BONUS,online,inzone)
				--	print ("perc:"..tostring(percentage))
					if eligible then
...
--                                                   name,                Log,SID,      n wGained,          newSpent,newHours,newCoeff,newReason, Activate, NoIncreaseVersion
						QDKP2log_SetEntry(QDKP2_GetMain(name),Log,SID, QDKP2timerBase.BONUS, nil,        nil,          nil,                nil,             nil,               true)
            SomeoneAwarded = true
          elseif noreason then
						QDKP2log_Entry(name, QDKP2_LOC_IntegerTime, QDKP2LOG_NODKP,  {QDKP2timerBase.BONUS, nil, nil},nil,QDKP2log_PacketFlags(nil,nil,nil,nil,noreason))
...
function QDKP2_IronManStart() --sets the ironman mark
  ...
  if QDKP2_ManagementMode() then
    ...
    for i=1, #QDKP2raid do -- Place IronMan marker on main
    QDKP2ironMan.PLAYERS[QDKP2_GetMain(QDKP2raid[i])] = true
    end
...
function QDKP2_InronManFinish(BonusDKP) --calculates who award the ironman bonus and award them BonusDKP
  ...
  for i=1, #playerList do
    ...
    elseif QDKP2_AltsStillToCome(name,playerList,i) then
      ...
      if noreason then
        QDKP2log_Entry(name, "Iron Man Bonus", QDKP2LOG_NODKP, {BonusDKP, nil, nil},timeStamp,QDKP2log_PacketFlags(true,nil,nil,nil,noreason))
        if QDKP2_AnnounceFailIM and QDKP2online[name] then
          local msg=QDKP2log_GetLastLogText(name)
          QDKP2_SendHiddenWhisper(msg,name)
        end
...
function QDKP2_GetEligibility(name,awardtype,award,online,inzone)
  ...
  if QDKP2_IsExternal(name) then
	...
	if percentage==9999999 or percentage==100 then percentage=nil; end
--	print (eligible,percentage, reason)
