...
local LO_VERSION	= '60200.1';
local LO_RELEASE_DATE = "May 1, 2016";
...
local LoadITDetails = 
{
	...
	author = "Ru (updated by Saur)",
	email = "pkj@axis.com",
	website = "http://wow.curse.com/downloads/wow-addons/details/loadit.aspx",
...
local LO_HELP = {
	LOADIT_VERSION .. ', by Ru (updated by Saur) :: Help Menu',
...
--	RegisterForSave("LoadITcf");
	...
	LoadIT_Print(LOADIT_VERSION .. ', by Ru (updated by Saur) :: Type ' .. LO_CYAN .. '/mods' .. LO_GREY .. ' for Help');
end

function LoadIT_OnEvent(self, event, ...)
	local arg1 = ...;
...
function LoadIT_ShowProfiles(type)
	...
	for key,_ in pairs(tmplist) do count = count + 1; end;
...
function LoadIT_LoadProfile(p, type)
	...
	for key,s in pairs(modules) do

		local name, title, notes, loadable, reason, security = GetAddOnInfo(key);
		local enabled = (GetAddOnEnableState(UnitName("player"), key) > 0);
...
function LoadIT_ShowProfile(p, type)
	...
	for key,enabled in pairs(modules) do
...
function LoadIT_Disable(p, silent, bulk)
	if (p) then
		...
		local name = GetAddOnInfo(p);
		local enabled = (GetAddOnEnableState(UnitName("player"), p) > 0);
		if (not enabled) then
...
function LoadIT_Enable(p, silent, bulk)
	if (p) then
		...
		local name, _, _, loadable, reason, _ = GetAddOnInfo(p);
		local enabled = (GetAddOnEnableState(UnitName("player"), p) > 0);
		if (enabled) then
...
function LoadIT_Load(p, silent)
	if (p) then
		local name, _, _, loadable, _, _ = GetAddOnInfo(p);
...
function LoadIT_command(msg)
	...
	elseif ((c == 'menu') or (LoadITcf.ShortcutMenu and (not c)))then
		...
		for key,s in pairs(LO_HELP) do
			LoadIT_Print(s);
		end
		if (LoadITmenu) then
			for key,s in pairs(LO_HELP2) do
				LoadIT_Print(s);
			end
		end
		LoadIT_Print('');
