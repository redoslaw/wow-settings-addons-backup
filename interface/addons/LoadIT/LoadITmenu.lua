--  Title:  LoadIT Menu
--  Author: Ru of Frostmane
--  Updated by: Saur of Emerald Dream (EU)
--
...
LOADITOPT_VERSION = LOADIT_VERSION .. ' by Ru (updated by Saur)';
...
function LoadITopt_OnLoad(self)
	self:SetBackdropBorderColor(TOOLTIP_DEFAULT_COLOR.r, TOOLTIP_DEFAULT_COLOR.g, TOOLTIP_DEFAULT_COLOR.b, 0.4);
	self:SetBackdropColor(TOOLTIP_DEFAULT_BACKGROUND_COLOR.r, TOOLTIP_DEFAULT_BACKGROUND_COLOR.g, TOOLTIP_DEFAULT_BACKGROUND_COLOR.b, 0.8);
...
function LoadITopt_OnShow(self)
	LoadITLive_OnUpdate();
	LoadITProfile_OnUpdate();
	PlaySound("igCharacterInfoOpen");
end

function LoadITopt_OnHide(self)
	LoadITEdit_Reset();
	LoadITEdit_OnUpdate();
	LoadITProfile_UpdateButtonData();
...
function LoadIToptTab_OnClick(self)
	local id = self:GetID();
	PanelTemplates_Tab_OnClick(self, LoadITmenuFrame);
...
function LoadITopt_OnMouseDown(self, button)
...
function LoadITopt_OnMouseUp(self, button)
	if (LoadITcf.LockMenu) then return; end
	if (button == "LeftButton") then
...
function LoadITLiveButton_OnClick(self)
	local i = self:GetID();
...
function LoadITProfileButton_OnClick(self)
	local i = self:GetID();
...
function LoadITEditButton_OnClick(self)
	local i = self:GetID();
...
function LoadITEdit_GetList()
	...
	local key, enabled;
	for key,enabled in pairs(tmplist) do
		local name, title, notes = GetAddOnInfo(key);
		local error = 0;
		if (not title) then
			name = key;
			error = 1;
	...
	local index;
		...
		for i = 1, LOADIT_SCROLL_LINES do
			...
			if (index <= table.getn(LoadITList)) then
				if (LoadITList[index].loaded) then
...
function LoadITLive_ToggleAll(arg)
	...
	for i = 1, table.getn(LoadITList) do
			if (arg == 1) then
				LoadITList[i].enabled = true;
				LoadIT_Enable(LoadITList[i].name, LoadITcf.Verbose, 1);
			else
				LoadITList[i].enabled = false;
...
function LoadIT_AddonTooltip(idx)
	local s;
	local txt = LO_YELLOW;
	local name, title, notes, loadable, reason, security = GetAddOnInfo(LoadITList[idx].name);
	local enabled = (GetAddOnEnableState(UnitName("player"), LoadITList[idx].name) > 0);
	...
	local tmp = "";
	if (notes) then
		for s in string.gmatch(notes, "%a+") do
...
function DDProfiles_OnLoad(self)
...
--	local i = 2;
--	for key,_ in pairs(LoadITcf.Classes) do
--		DDProfileList[i] = key;
--		i = i + 1;
--	end;
	...
	UIDropDownMenu_SetText(DDProfiles, 'View: ' .. DDProfileList[LoadITglo.View] .. ' Profiles');

	UIDropDownMenu_SetWidth(DDProfiles, 190);
	UIDropDownMenu_SetButtonWidth(DDProfiles, 190);
end

function DDProfiles_OnShow(self)
	DDProfiles_OnLoad(self);
end

function DDProfiles_Initialize(self)
	local info;
	local i;
...
function DDProfiles_OnClick(self)
	local id = self:GetID();
	UIDropDownMenu_SetSelectedID(DDProfiles, id);
	UIDropDownMenu_SetText(DDProfiles, 'View: ' .. DDProfileList[id] .. ' Profiles');
