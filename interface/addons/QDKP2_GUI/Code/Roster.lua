...
QDKP2GUI_Default_QuickPerc2				= 100
QDKP2GUI_Default_ShowOutGuild 			= true
QDKP2GUI_Default_CustomPlayerRosterList = {}
local myClass={}
...
function myClass.Refresh(self, forceResort)
	...
	QDKP2frame2_selectList_Custom:SetChecked(false)
    ...
    elseif self.Sel=="bid" then
      ...
      QDKP2_Frame2_Bid_Item:SetText(QDKP2_BidM.ITEM or '')
    ...
    for i=1, QDKP2GUI_Roster.ENTRIES do  --fills in the list data
      ...
      if indexAt <= #self.List then
		...
		--[[Debugging block for cross-realm
	 
		DEFAULT_CHAT_FRAME:AddMessage("RosterName "..name) 
        ...
        _G[ParentName.."_name"]:SetVertexColor(r, g, b, a)
        _G[ParentName.."_roll"]:SetVertexColor(r, g, b, a)
        _G[ParentName.."_bid"]:SetVertexColor(r, g, b, a)
        _G[ParentName.."_value"]:SetVertexColor(r, g, b, a)
        _G[ParentName.."_rank"]:SetVertexColor(r, g, b, a)
        local classColor=colors
        if not QDKP2_USE_CLASS_BASED_COLORS then
          classColor=QDKP2_GetClassColor(class)
        end
        _G[ParentName.."_class"]:SetVertexColor(classColor.r, classColor.g, classColor.b, a)
        if isinguild and QDKP2_GetNet(name)<0 then
          _G[ParentName.."_net"]:SetVertexColor(1, 0.2, 0.2)
        else
          _G[ParentName.."_net"]:SetVertexColor(r, g, b, a)
        end
        _G[ParentName.."_total"]:SetVertexColor(r, g, b, a)
        _G[ParentName.."_spent"]:SetVertexColor(r, g, b, a)
        _G[ParentName.."_hours"]:SetVertexColor(r, g, b, a)
        _G[ParentName.."_deltatotal"]:SetVertexColor(r, g, b, a)
        _G[ParentName.."_deltaspent"]:SetVertexColor(r, g, b, a)
        ...
        _G[ParentName.."_name"]:SetText(tostring(nameS));
        _G[ParentName.."_roll"]:SetText(tostring(roll or '-'))
        _G[ParentName.."_bid"]:SetText(tostring(bid or '-'))
        _G[ParentName.."_value"]:SetText(tostring(value or '-'))
        _G[ParentName.."_rank"]:SetText(tostring(rank or '-'));
        _G[ParentName.."_class"]:SetText(tostring(class or '-'));
        _G[ParentName.."_net"]:SetText(tostring(net or '-')..DKP_Ast);
        _G[ParentName.."_total"]:SetText(tostring(total or '-')..DKP_Ast);
        _G[ParentName.."_spent"]:SetText(tostring(spent or '-')..DKP_Ast);
        _G[ParentName.."_hours"]:SetText(tostring(hours or '-'));
        _G[ParentName.."_deltatotal"]:SetText(tostring(s_gain or '-'));
        _G[ParentName.."_deltaspent"]:SetText(tostring(s_spent or '-'));


        if self:isSelectedPlayer(name) then _G[ParentName.."_Highlight"]:Show()
        else _G[ParentName.."_Highlight"]:Hide()
        end
        _G[ParentName]:Show();
      else
        _G[ParentName]:Hide();
...
function myClass.PupulateList(self)
  ...
  elseif self.Sel=='bid' then
	...
	if QDKP2GUI_HideAlts and self.Sel ~= 'bid' then
		local temp={}
		for i,name in pairs(self.List) do
			if not QDKP2_IsAlt(name) then table.insert(temp,name); end
		end
		self.List=temp
	end
...
function myClass.ShowColumn(self, Column, todo)
  ...
  local expand, reduce
  TestObj=_G["QDKP2_Frame2_SortBtn_"..Column]
  ...
  for i=1, QDKP2GUI_Roster.ENTRIES do
    local ParentName="QDKP2_frame2_entry"..tostring(i)
    local ColObj=_G[ParentName..'_'..Column]
    if todo then
      --ColObj:Show()
    ...
    local RowObj=_G[ParentName]
    if reduce then RowObj:SetWidth(RowObj:GetWidth()-(width))
    elseif expand then RowObj:SetWidth(RowObj:GetWidth()+(width))
    end
  end
  local TitleColObj=_G["QDKP2_frame2_title_"..Column]
  local SortButton=_G["QDKP2_Frame2_SortBtn_"..Column]
  if todo then
    --TitleColObj:Show()
...
function myClass.LeftClickEntry(self, tname)
  local t3name = tname:GetName()
  --DEFAULT_CHAT_FRAME:AddMessage("t3name is "..t3name)
  local name,btnIndex=QDKP2GUI_GetClickedEntry(tname, myClass)
  if IsShiftKeyDown() then
    ...
    elseif QDKP2GUI_IsDoubleClick(tname, myClass) and QDKP2_IsInGuild(name) then
...
function myClass.RightClickEntry(self, tname)
  local name=QDKP2GUI_GetClickedEntry(tname, myClass, suffix)
  --DEFAULT_CHAT_FRAME:AddMessage("right click name is "..name)
...
--selected state of name.
  QDKP2_Debug(1, "GUI-Roster","Selecting"..tostring(name))
...
{text="Subtract 25% DKP",
func=function()
  QDKP2_PlayerSpends(myClass.SelectedPlayers,"25%")
  QDKP2GUI_CloseMenus()
  QDKP2_RefreshAll()
end
},
...
checked=function() return QDKP2GUI_HideAlts; end,
func=function()
	if QDKP2GUI_HideAlts then QDKP2GUI_HideAlts=false
	else QDKP2GUI_HideAlts=true
	end
  myClass:ChangeList(myClass.Sel)
end,
},
ShowOnlyOnline={text="Hide offline members",
checked=function() return QDKP2GUI_HideOffline; end,
func=function()
	if QDKP2GUI_HideOffline then QDKP2GUI_HideOffline=false
	else QDKP2GUI_HideOffline=true
	end
  myClass:ChangeList(myClass.Sel)
end,
},
AddPlayersToCustom={text="Add members to custom roster",
func=function()
  ActualCustom=DictFromList(QDKP2GUI_Vars.CustomPlayerRosterList)
  for i,name in pairs(myClass.SelectedPlayers) do
    if not ActualCustom[name] then table.insert(QDKP2GUI_Vars.CustomPlayerRosterList,name); end
  end
end,
},
RemPlayersFromCustom={text="Remove members from custom roster",
func=function()
  RemoveDict=DictFromList(myClass.SelectedPlayers)
  for i,name in pairs(QDKP2GUI_Vars.CustomPlayerRosterList) do 
	--DEFAULT_CHAT_FRAME:AddMessage("remove i "..i.." name "..name)
	--if RemoveDict[name] then DEFAULT_CHAT_FRAME:AddMessage("YES") else DEFAULT_CHAT_FRAME:AddMessage("NO"); end
    --if RemoveDict[name] then table.remove(QDKP2GUI_Vars.CustomPlayerRosterList,i); end  --Old line
	--Dictionary from list not working, can't find it, hand implement
	local j, name2
	for j,name2 in pairs (myClass.SelectedPlayers) do
		if name==name2 then table.remove(QDKP2GUI_Vars.CustomPlayerRosterList,i); end
	end
  end
  myClass:ChangeList(myClass.Sel)
end,
},
CleanCustom={text="Clear custom roster",
  func=function() table.wipe(QDKP2GUI_Vars.CustomPlayerRosterList); myClass:ChangeList(myClass.Sel); end
},
Export={text="Export DKP values",
func=function() QDKP2_Export_Popup('list','text',myClass.List); end,
},
ExportSelected={text="Export DKP values for selected members",
func=function() QDKP2_Export_Popup('list','text',myClass.SelectedPlayers); end,
},
...
CloseBid={text="Close bidding",
func=QDKP2_BidM_CloseBid,
},
...
function myClass.PlayerMenu(self,List)
  ...
  table.insert(menu,LogVoices.ExportSelected)
  --DEFAULT_CHAT_FRAME:AddMessage("playermenu self.Sel "..self.Sel)
  if self.Sel=="custom" then
    table.insert(menu,LogVoices.RemPlayersFromCustom)
	table.insert(menu,LogVoices.CleanCustom)
  else
    table.insert(menu,LogVoices.AddPlayersToCustom)
  end
  table.insert(menu,LogVoices.spacer)
  table.insert(menu,LogVoices.MenuClose)

  if #sel==1 and not QDKP2_IsInGuild(sel[1]) then
    menu={} --external player, only the following voices shall be used.
  ...
  elseif #sel==1 then
    ...
    table.insert(menu,1,{text=name.."'s menu",isTitle=true})
    ...
    if QDKP2_IsExternal(name) then
      table.insert(menu,2,LogVoices.ExternalRem)
    end
    if self.Sel=="bid" then
      table.insert(menu,2,LogVoices.spacer)
      table.insert(menu,2,LogVoices.CancelBid)
      table.insert(menu,2,LogVoices.SetWinner)
    end
  elseif #sel>1 then
    table.insert(menu,1,{text="GROUP's actions:",isTitle=true})
    if #sel==2 then table.insert(menu,2,LogVoices.AltMake); end
		if managing and (self.Sel=='guild') then table.insert(menu,2,LogVoices.AllStandbyAdd); end
    if self.Sel=="bid" then
      table.insert(menu,2,LogVoices.spacer)
      table.insert(menu,2,LogVoices.CancelBid)
    end
...
function myClass.RosterMenu(self)
  ...
  if self.Sel=="guild" then
    ...
    table.insert(menu,2,LogVoices.spacer)
    table.insert(menu,2,LogVoices.ShowOnlyOnline)
    table.insert(menu,2,LogVoices.HideAlts)
    if QDKP2_OfficerMode() then
  ...
  elseif self.Sel=="custom" then
    table.insert(menu,2,LogVoices.spacer)
    table.insert(menu,2,LogVoices.CleanCustom)
    table.insert(menu,2,LogVoices.ShowOnlyOnline)
    table.insert(menu,2,LogVoices.HideAlts)
  elseif self.Sel=="bid" then
    ...
    if QDKP2_BidM_isBidding() then
      ...
      --1table.insert(menu,2,LogVoices.CloseBid)
    end
    table.insert(menu,3,LogVoices.ShowOutGuild)
  end

  table.insert(menu,LogVoices.spacer)
	table.insert(menu,LogVoices.Export)
