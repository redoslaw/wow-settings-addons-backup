...
function myClass.Refresh(self,doNotUpdate)
  ...
  for i=1,myClass.ENTRIES do
    ...
    if (indexAt <= #myClass.List) then
      ...
      _G[ParentName.."_date"]:SetText(Voice.Time);
      if Voice.DKPhist then _G[ParentName.."_net"]:SetText(Voice.DKPhist);
      else _G[ParentName.."_net"]:SetText("")
      end
      if Voice.DKPchange then _G[ParentName.."_mod"]:SetText(Voice.DKPchange);
      else _G[ParentName.."_mod"]:SetText("");
      end
      local description=Voice.Desc or "NIL"
      if Voice.isChild then description="   "..description; end
      _G[ParentName.."_action"]:SetText(description);
      ...
      _G[ParentName.."_date"]:SetVertexColor(colors.r, colors.g, colors.b, 1)
      _G[ParentName.."_action"]:SetVertexColor(colors.r, colors.g, colors.b, 1)
      _G[ParentName.."_net"]:SetVertexColor(netColors.r, netColors.g, netColors.b, 1)
      _G[ParentName.."_mod"]:SetVertexColor(amountColors.r, amountColors.g, amountColors.b, 1)
      local expandBtn=_G[ParentName.."_expand"]
      if Voice.SubList then
        expandBtn:Show()
        if myClass.Expanded[Voice.ID] then expandBtn:SetNormalTexture("Interface\\Buttons\\UI-MinusButton-Up")
        else expandBtn:SetNormalTexture("Interface\\Buttons\\UI-PlusButton-Up")
        end
      else
        expandBtn:Hide()
      end
      _G[ParentName]:Show();
      if myClass.Selected==Voice.ID then _G[ParentName.."_Highlight"]:Show()
...
function myClass.LeftClick(self, tname)
  QDKP2GUI_CloseMenus()
  local Voice=QDKP2GUI_GetClickedEntry(tname, myClass)
  if IsShiftKeyDown() and ChatFrameEditBox:IsVisible() then
    ChatFrameEditBox:Insert((Voice.Time or 'Unknown timestamp')..' - '..(Voice.Desc or "NIL"))
    return
  elseif QDKP2_OfficerMode() and QDKP2GUI_IsDoubleClick(tname,myClass) then
    myClass:VoiceMenu(Voice,true)
  end
  myClass:SelectVoice(Voice)
end

function myClass.RightClick(self, tname)
  QDKP2GUI_CloseMenus()
  local Voice=QDKP2GUI_GetClickedEntry(tname, myClass)
  myClass:SelectVoice(Voice)
...
function myClass.ExpandToggle(self, LogLine, ForceVoice)
 QDKP2GUI_CloseMenus()
 Voice=ForceVoice or QDKP2GUI_GetClickedEntry(LogLine, myClass,"_expand")
...
dclick=true,
func=function() myClass:ExpandToggle(nil, myClass.MenuVoice); end,
},
...
ExpandAll={text="Expand All", func=myClass.ExpandAll},
...
function myClass.WindowMenu(self)
  ...
  table.insert(menu,LogVoices.ExpandAll)
