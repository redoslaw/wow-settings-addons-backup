...
if u.SkipOrUpgrade(c, "OptionsFrames", tonumber("20150310040701") or time()) then
   return
end
...
function c.SetOption(name, value)
   ...
   if c.A.Options[name].OnSet then
      c.A.Options[name].OnSet(value)
   end
end
...
function c.AddRotationSwitches()
   ...
   options["MinimapIcon"] = {
      ...
      Default = true,
      OnSet = function() c.UpdateMinimapIcon() end
   }
end
