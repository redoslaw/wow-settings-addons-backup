...
function PassLoot:ShowTooltip(...)
  ...
  for i = 2, select("#", ...) do
    GameTooltip:AddLine(PassLoot.FontGold..select(i, ...));
  end
  GameTooltip:Show();
end
...
function PassLoot:BuildUnknownVars()
...
function PassLoot:Rules_ActiveFilters_OnScroll()
  ...
  local Line, LineNum;
  local NumLines = 0;
  local WidgetKey, Offset, Text, NumFilters, KnownVar;
...
function PassLoot:CopyCurrentRuleToProfile(profile)
  local self = PassLoot;  -- as I will be using this from PassLoot.CopyCurrentRuleToProfile and the first arg will be the frame
  if ( self.CurrentRule > 0 ) then
