
QDKP2_Config_DB = {
	["char"] = {
		["Redbullek - Burning Legion"] = {
			["ActiveProfiles"] = {
				["Default"] = "Default",
				["Burning Legion-Wipeforged"] = "Burning Legion-Wipeforged",
			},
		},
		["Ðeathshot - Steamwheedle Cartel"] = {
			["ActiveProfiles"] = {
				["Default"] = "Default",
			},
		},
		["Redus - Burning Legion"] = {
			["ActiveProfiles"] = {
				["Default"] = "Default",
				["Burning Legion-Wipeforged"] = "Burning Legion-Wipeforged",
			},
		},
		["Ðeadshot - Burning Legion"] = {
			["ActiveProfiles"] = {
				["Default"] = "Default",
				["Burning Legion-Wipeforged"] = "Burning Legion-Wipeforged",
			},
		},
	},
	["profileKeys"] = {
		["Redbullek - Burning Legion"] = "Burning Legion-Wipeforged",
		["Ðeathshot - Steamwheedle Cartel"] = "Default",
		["Redus - Burning Legion"] = "Burning Legion-Wipeforged",
		["Ðeadshot - Burning Legion"] = "Burning Legion-Wipeforged",
	},
	["profiles"] = {
		["Default"] = {
		},
		["Burning Legion-Wipeforged"] = {
			["BM_AllowMultiple"] = false,
			["BM_AckBets"] = false,
			["AW_IM_InWhenStarts"] = false,
			["BM_LogBets"] = false,
			["AW_TIM_RaidLogTicks"] = false,
			["BM_AnnounceStart"] = false,
			["BM_Countdown"] = false,
			["BM_OutGuild"] = false,
			["BM_ConfirmWinner"] = false,
			["MISC_Inf_WentNeg"] = false,
			["BM_AnnounceWinner"] = false,
			["BM_AutoRoll"] = false,
			["BM_GetWhispers"] = false,
			["MISC_UploadOn_Payment"] = false,
			["BM_RoundValue"] = false,
			["BM_GiveToWinner"] = false,
			["BM_AckReject"] = false,
			["MISC_UploadOn_Hourly"] = false,
			["ODS_Enable"] = false,
			["FOR_CompactNote"] = true,
			["BM_GetGroup"] = false,
			["BM_CatchRolls"] = false,
			["MISC_UploadOn_ZS"] = false,
			["AW_IM_PercReq"] = 100,
			["MISC_UploadOn_Raid"] = false,
			["FOR_StoreHours"] = false,
			["BM_AnnounceCancel"] = false,
			["AN_AnAwards"] = false,
		},
	},
}
